  ...
  return (
    <div className='flex flex-col sm:flex-row gap-1 sm:gap-10 pt-10 border-t'>
      ...
      <div className='min-w-60'>
        <p onClick={() => setShowFilter(!showFilter)} className={`my-2 text-xl flex items-center cursor-pointer gap-2 ${isDark ? 'text-white' : 'text-gray-900'}`}>
          FILTERS
          <img className={`h-3 sm:hidden ${showFilter ? ' rotate-90' : ''}`} src={assets.dropdown_icon} alt="" />
        </p>

        {/* Category Filter */}
        <div className={`border pl-5 py-3 mt-6 ${showFilter ? '' : 'hidden'} sm:block ${isDark ? 'border-gray-600' : 'border-gray-300'}`}>
          <p className={`mb-3 text-sm font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>CATEGORIES</p>
          <div className={`flex flex-col gap-2 text-sm font-light ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>
        ...
        {/* Sub Category Filter */}
        <div className={`border pl-5 py-3 my-5 ${showFilter ? '' : 'hidden'} sm:block ${isDark ? 'border-gray-600' : 'border-gray-300'}`}>
          <p className={`mb-3 text-sm font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>TYPE</p>
          <div className={`flex flex-col gap-2 text-sm font-light ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>
      ...
      <div className='flex-1'>
        <div className='flex justify-between text-base sm:text-2xl mb-4'>
          ...
          <select 
            ...
            className={`border-2 text-sm px-2 ${isDark ? 'border-gray-600 bg-gray-800 text-white' : 'border-gray-300 bg-white text-gray-900'}`} 
            name="" 
            id=""
          >
